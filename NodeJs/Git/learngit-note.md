...
**补充**  `pwd`显示当前路径，`ls`显示当前目录，`ls -ah`显示隐藏目录
#### 在当前目录初始化库
```
$ git init
```
...
工作区就是电脑中能看到的目录，工作区有一个隐藏目录 `.git`，这是Git的`版本库`

`git add` 将修改添加到暂存区，`git commit` 将修改添加到当前分支。
...
```
git status
git log --pretty=oneline // 查看提交的版本信息
git reset --hard commit_id
git reflog // 查看所有分支以及操作记录包括删除的
```
