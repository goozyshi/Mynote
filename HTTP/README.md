...
### HTTP
  ...
  - 端口：80
  - 通信快，使用明文通信
  - 不验证通信方身份，也无法验证报文完整性，从而容易遭受中间人攻击
  - 中间人攻击

    攻击方同时与服务端和客户端建立起了连接，并让对方认为连接是安全的，但是实际上整个通信过程都被攻击者控制了。攻击者不仅能获得双方的通信信息，还能修改通信信息。`公共WIFI`
  ![](./demoPic/mitm.png)
### HTTPS
...
- `XSS`跨站脚本攻击
  简单点来说，就是攻击者想尽一切办法将可以执行的代码注入到网页中。
    ...
    如何防御：
    - 转义字符： 对于引号、尖括号、斜杠进行转义;
    - 建立白名单： 对`HTTP Header`做限制：1.只允许加载本站资源 2. 只允许加载 HTTPS 协议图片等
- `CSRF`跨站点请求伪造
